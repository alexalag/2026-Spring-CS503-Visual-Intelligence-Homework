...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder = TransformerTrunk(
            dim=dim, depth=enc_depth,
            mlp_ratio=mlp_ratio, head_dim=head_dim,
            use_bias=use_bias
        ) # Define the Transformer encoder trunk

        self.decoder = TransformerDecoderTrunk(
            dim=dim, depth=dec_depth,
            mlp_ratio=mlp_ratio, head_dim=head_dim,
            use_bias=use_bias
        ) #  Define the Transformer decoder trunk

        # Initialize encoder -> decoder context projection
        self.dec_context_proj = nn.Linear(dim, dim)  # Define the encoder -> decoder projection. This is simply a linear layer mapping dim -> dim.

        # Initialize decoder output projection
        self.to_logits = nn.Linear(dim, self.vocab_size, bias=False) # Define the output projection layer using self.vocab_size
    ...
    def generate_one_modality_roar(
        ...
        chunks = torch.split(shuffled_positions, schedule)  # splits into num_steps chunks
        ...
        dec_input_positions_list = [chunk.unsqueeze(0) for chunk in chunks] 
        ...
        for step, k in enumerate(schedule):
            ...
            enc_input_tokens     = torch.cat([enc_input_tokens,     samples.unsqueeze(0)],          dim=1)  # [1, N+k]
            enc_input_positions  = torch.cat([enc_input_positions,  dec_input_positions],            dim=1)  # [1, N+k]
            enc_input_modalities = torch.cat([enc_input_modalities, dec_input_modalities],           dim=1)  # [1, N+k]
