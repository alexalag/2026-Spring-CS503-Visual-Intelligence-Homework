...
class MaskGIT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        tokens = tokens + self.positional_embedding[:L, :]
        ...
        logits = self.to_logits(self.out_norm(tokens))
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Avoiding to use a for loop here, and instead use vectorized operations?
        # Mask created on the same device as seq.

        # Sample how many tokens to mask per sample
        num_masked = torch.randint(1, L + 1, (B,), device=seq.device) 
        # Generate random scores for every position in every sample
        rand_scores = torch.rand(B, L, device=seq.device)
        # Compute the rank of each position
        sorted_indices = rand_scores.argsort(dim=1) 
        ranks = sorted_indices.argsort(dim=1)
        # Mask positions with the lowest ranks 
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        # Ignore the ignore_index in the loss calculation
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            topk_confidence_indices = confidence.topk(k).indices
            selected_positions = masked_indices[topk_confidence_indices]
